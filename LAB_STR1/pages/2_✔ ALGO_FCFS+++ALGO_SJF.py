...
def make_dataframe(process, start, burst, finish, turn_around, wait):
    ...
    """
    df = pd.DataFrame({"Process": process, 'Arrival Time': start, "Burst Time": burst, 'Completion Time': finish,
                       'Turn Around Time': turn_around, 'Waiting Time': wait}).sort_values(by='Process')
    return df
...
def round_robin(pro, time_quanta):
    ...
    while len(temp_process) != 0 or len(queue) != 0:
        if len(queue) == 0:  # no proccesse have arrived on given cpu time
            ...
            if queue[0][
                1] > time:  # if it's arrival time is larger than cpu_time then CPU will be empty from time to arrival_time of process
...
def app_layout():
    ...
    st.title("Algorithmes de planification du processeur : FCFS et SJF")
    ...
    if 'Round Robin' in algo:
        ...
        st.write(
            "RR is a CPU scheduling algorithm where each process is assigned a fixed time slot in a cyclic way, where each process gets equal share in time processing. RR is a hybrid model which is clock-driven. In this, CPU is shifted to the next process after fixed interval time, which is called time quantum/time slice. Time slice should be minimum, which is assigned for a specific task that needs to be processed. However, it may differ OS to OS.")
        ...
        st.write(
            "Gantt chart seems to come too big (if quantum time is less for scheduling.For Example:1 ms for big scheduling.)There is low throughput and context switches.Lower time quantum results in higher the context switching overhead in the system.Finding a correct time quantum is a quite difficult task in this system.If slicing time of OS is low, the processor output will be reduced.")
        time_quanta = st.number_input(label="Time Quantum", min_value=1, max_value=10, step=1,
                                      key="RR_quantum")  # input for time quantum
    ...
    for i, x in enumerate(cols):
        with x:
            ...
            temp = ['P{}'.format(i), 0, 0]
            temp[1] = st.number_input(label="Arrival Time", min_value=0, max_value=100, value=i, step=1,
                                      key="AT{}".format(i))
            temp[2] = st.number_input(label="Burst Time", min_value=1, max_value=100, value=i + 1, step=1,
                                      key="BT{}".format(i))
            if 'Priority Scheduling' in algo:
                temp.append(st.number_input(label="Priority", min_value=1, max_value=10, value=i + 1, step=1,
                                            key="Priority{}".format(i)))
            process.append(temp)
